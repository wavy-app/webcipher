...
use crate::key_stores::remote::key::Key;
use crate::prelude;
...
/// [`decode`](`RemoteStore::decode`). If the token was indeed provisioned by
/// the target, this operation will be successful. Otherwise, it will fail.
///
...
/// let key_store = RemoteStore::new(uri).await?;
///
/// let token = "a.b.c";
...
/// certain period of time. [`RemoteStore`] provides the
/// [`refresh`](`RemoteStore::refresh`) function to refresh a given
/// [`RemoteStore`]. [`refresh`](`RemoteStore::refresh`) will re-fetch the new
...
pub struct RemoteStore {
    ...
    ///
    /// Two [`RemoteStore`]'s are considered equivalent if and only if their
    /// `uri`'s match.
...
impl RemoteStore {
    /// Generate a new [`RemoteStore`] by asynchronously fetching the keys at
    /// the given [`URI`].
    ///
    /// [`URI`]: https://docs.rs/http/latest/http/uri/struct.Uri.html
    ...
    /// Refreshes the current [`RemoteStore`] by asynchronously fetching the
        ...
        Claim: for<'a> Deserialize<'a>,
    {
        ...
        let ref token: String = token.into();
        ...
        let ref validation = Validation::new(alg);
...
async fn fetch(uri: http::Uri) -> prelude::Result<HashMap<String, Key>> {
...
) -> HashMap<String, (Key, DecodingKey)> {
    key_map
        .into_iter()
        .filter_map(|(kid, key)| {
            let Key { e, n, .. } = &key;
...
// #[test]
// fn test() {
//     let token = "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.b.c";
//     let x = jsonwebtoken::decode_header(token).unwrap();
//     println!("{:#?}", x);
// }
